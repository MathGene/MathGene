...
var mgCalc = (function() {
	...
	function xprSolve(xSol,cVar) {//solve equation xSol in MG format for variable cVar
		var solverMap = 
		{
		"sin":{"solverU":"asnS(lExpr)"},
		"cos":{"solverU":"acsS(lExpr)"},
		"tan":{"solverU":"atnS(lExpr)"},
		"sec":{"solverU":"ascS(lExpr)"},
		"csc":{"solverU":"accS(lExpr)"},
		"cot":{"solverU":"actS(lExpr)"},
		"snh":{"solverU":"ashS(lExpr)"},
		"csh":{"solverU":"achS(lExpr)"},
		"tnh":{"solverU":"athS(lExpr)"},
		"sch":{"solverU":"axhS(lExpr)"},
		"cch":{"solverU":"ayhS(lExpr)"},
		"cth":{"solverU":"azhS(lExpr)"},
		"asn":{"solverU":"sinS(lExpr)"},
		"acs":{"solverU":"cosS(lExpr)"},
		"atn":{"solverU":"tanS(lExpr)"},
		"asc":{"solverU":"secS(lExpr)"},
		"acc":{"solverU":"cscS(lExpr)"},
		"act":{"solverU":"cotS(lExpr)"},
		"ash":{"solverU":"snhS(lExpr)"},
		"ach":{"solverU":"cshS(lExpr)"},
		"ath":{"solverU":"tnhS(lExpr)"},
		"axh":{"solverU":"schS(lExpr)"},
		"ayh":{"solverU":"cchS(lExpr)"},
		...
		"int":{"solverU":""},
		"abs":{"solverU":""},
		"erf":{"solverU":"efcS(lExpr)"},
		"efc":{"solverU":"erfS(lExpr)"},
		"arg":{"solverU":""},
		"exp":{"solverU":"lneS(lExpr)"},
		"con":{"solverU":"conS(rExpr)"},
		"gam":{"solverU":""},
		"cdf":{"solverU":""},
		"pdf":{"solverU":""},
		"lcf":{"solverU":""},
		...
		"vec":{"solverU":"vecS(rExpr)"},
		"hat":{"solverU":"hatS(rExpr)"},
		"und":{"solverU":"undS(rExpr)"},
		...
		"lgn":{"solverU":"cDivS(lneS(strgI),lneS(lExpr))","solverL":"cPowS(lExpr,strgI)"},
		"cPow":{"solverU":"cPowS(lExpr,(cDivS(1,strgI)))","solverL":"lgnS(strgI,lExpr)"},
		"cMul":{"solverU":"cDivS(lExpr,strgI)","solverL":"cDivS(lExpr,strgI)"},
		"cTms":{"solverU":"cDivS(lExpr,strgI)","solverL":"cDivS(lExpr,strgI)"},
		"cDiv":{"solverU":"cMulS(lExpr,strgI)","solverL":"cDivS(strgI,lExpr)"},
		"cAdd":{"solverU":"cSubS(lExpr,strgI)","solverL":"cSubS(lExpr,strgI)"},
		"cSub":{"solverU":"cAddS(lExpr,strgI)","solverL":"cSubS(strgI,lExpr)"},
		};
		function cCrawl(lExpr,rExpr,xVar) {
			if (rExpr.split(")").length-1 != rExpr.split("(").length-1) {return cError("<span style='color:red'>Unmatched parentheses/brackets</span>")}
			rExpr = cReduce(rExpr);
			while (strTest(rExpr,"(")) {
				...
				else if (typeof eval("solverMap."+getOp.func+".solverL") != "undefined") {
					if (strTest(getOp.upper,xVar) && strTest(getOp.lower,xVar)) {break}
					else if (strTest(getOp.upper,xVar)) {
						rExpr = getOp.upper;
						var strgI = getOp.lower;
						lExpr = eval(eval("solverMap."+getOp.func+".solverU"));
					}
					else if (strTest(getOp.lower,xVar)) {		
						rExpr = getOp.lower;
						var strgI = getOp.upper;
						lExpr = eval(eval("solverMap."+getOp.func+".solverL"));
					}
					else {break}
				}
				else {
					if (eval(eval("solverMap."+getOp.func+".solverU")) == "") {break}
					else {lExpr = eval(eval("solverMap."+getOp.func+".solverU"));rExpr = getOp.upper}
				}
			}
			lExpr += "";
			return [rExpr,lExpr];
		...
		//
		xSol = xSol.replace("Cv[10032]","");//remove spaces
		if (!strTest(xSol,"=")) {return mgExport(cReduce(cFunc(parseCalculus(xSol))))}
		var xSplit = xSol.split("=");
		if (xSplit.length > 2) {return cError("<span style='color:red'>Too many ='s</span>")}
		if (xSplit[0] == "" || xSplit[1] == "") {return cError("<span style='color:red'>Missing operand(s)</span>")}
		solverFlag = true;
		var rightXpr = cReduce(cSubS(cFunc(parseCalculus(xSplit[1])),cFunc(parseCalculus(xSplit[0]))));
		var cRet = cCrawl("0",rightXpr,cVar);
		solverFlag = false;
		var sReturn = mgExport(cReduce(cRet[0]))+"="+mgExport(cReduce(cRet[1]));
		if (cInventory(rightXpr).length != cInventory(sReturn).length && deeVarP == cVar) {return "undefined"}
		if (strTest(sReturn,"Cv[9998]")) {return "undefined"}
		return sReturn
	}
	...
	function strTest(xTarget,xSearch) { //test if xSearch string is in xTarget
		...
		if (nbrTest(xTarget) || xTarget.indexOf(xSearch) == -1) {return false};
		return true
	}
	...
	function cIterate(xIter) {
		...
		if (xIter.search(/,,/) > -1 || xIter.search(/,\)/) > -1 || xIter.search(/\(,/) > -1 || xIter.search(/\(\)/) > -1) {return cError("<span style='color:red'>Missing operand(s)</span>")}
		return eval(xIter.replace(/([a-z])\(/g,"$1S(").replace(/(Cv\[\d+\])/g,"'$1'").replace(/(Pv\[\d+\])/g,"'$1'").replace(/(Sv\[\d+\])/g,"'$1'"))+"";
	}
	...
	function xReduce(xRdce) { //basic expression reduction
		xRdce += "";
		if (strTest(xRdce,"undefined")) {return "undefined"}
		var pxpTemp = pxpFlag;
		pxpFlag = true;
		xRdce = cIterate(xRdce);
		Pv = [""];
		xRdce = cIterate(cIterate(xRdce.replace(/cMul\(/g,"vMul(").replace(/cDiv\(/g,"vDiv(").replace(/cNeg\(/g,"vNeg(")).replace(/Pv\[(\d+)\]/g,"pxp($1)")); //reduce product-quotient-exponent terms
		pxpFlag = false;Sv = [""];
		xRdce = cIterate(cIterate(xRdce.replace(/cAdd\(/g,"vAdd(").replace(/cSub\(/g,"vSub(")).replace(/Sv\[(\d+)\]/g,"smx($1)")); //reduce sum-difference terms
		Pv = [""];
		xRdce = cIterate(cIterate(xRdce.replace(/cMul\(/g,"vMul(").replace(/cDiv\(/g,"vDiv(").replace(/cNeg\(/g,"vNeg(")).replace(/Pv\[(\d+)\]/g,"pxp($1)")); //reduce product-quotient-exponent terms (slight return)
		pxpFlag = pxpTemp;
		return xRdce
	}
	function iReduce(iRdce) { //reduce by iterative expansion
		iRdce += "";
		if (strTest(iRdce,"undefined")) {return "undefined"}
		var iInv = cDissect(iRdce);
		var iCount = [""];
		var xFlag = 0;
		for (var iC=0;iC<iInv.length;iC++) {if (nbrTest(iInv[iC]) || iInv[iC] == "" || !varTest(iInv[iC])) {iCount[iC] = 0} else {iCount[iC] = iRdce.split(iInv[iC]).length-1};if (iCount[iC] > 1) {xFlag = 1}}
		if (xFlag == 0) {return iRdce}
		var iTemp = iExpand(iRdce);
		if (iTemp == iRdce || strTest(iTemp,"undefined")) {return iRdce}
		var iTempTmp = xReduce(iTemp);
		if (iTempTmp != iTemp && !strTest(iTempTmp,"undefined")) {iTemp = iTempTmp}
		for (var iC=0;iC<iInv.length;iC++) {
			if (iTemp.split(iInv[iC]).length-1 >= iCount[iC] && iCount[iC] != 0) {return iRdce}
			if (iTemp.split(iInv[iC]).length-1 < iCount[iC] && iCount[iC] != 0) {return iTemp}		
		}
		return iRdce
	}
	function smxS(xU) { //consolidate sum-difference terms 2a-b+c/d -> [(2a),(-b),(c/d)]
		Sv[xU] += "";
		for (var xI=0;xI<Sv.length;xI++) {Sv[xU] = Sv[xU].replace("Sv["+xI+"]_",Sv[xI]+"_").replace("Sv["+(xI-1)+"]_",Sv[xI-1]+"_").replace("Sv["+(xI-2)+"]_",Sv[xI-2]+"_")} //recover first level pointers 		
		for (var xI=0;xI<Sv.length;xI++) {for (var yI=0;yI<Sv.length;yI++) {if (xI != yI && Sv[xI] == Sv[yI] && Sv[xI] != "") {var rgx = new RegExp("Sv\\["+yI+"\\]","g");Sv[xU] = Sv[xU].replace(rgx,"Sv["+xI+"]")}}} //consolidate identical pointers
		...
		for (var xI=0;xI<sTerms.Terms.length;xI++) {
			if (sTerms.Terms[xI].search(/Cv\[111\d+\]/) > -1) {fTempC = cAddS(cDivS(cMulS(sTerms.Factors[xI],sTerms.Terms[xI]),sTerms.Divisors[xI]),fTempC)} //collect constants of integration 
	...
	function pxpS(xU) { //consolidate product-quotient-exponent terms 2ab/(cd) -> [(2),(a),(b),(c^-1),(d^-1)]
		Pv[xU] += "";
		for (var xI=0;xI<Pv.length;xI++) {Pv[xU] = Pv[xU].replace("Pv["+xI+"]_",Pv[xI]+"_").replace("Pv["+(xI-1)+"]_",Pv[xI-1]+"_").replace("Pv["+(xI-2)+"]_",Pv[xI-2]+"_")} //recover first level pointers 
		for (var xI=0;xI<Pv.length;xI++) {for (var yI=0;yI<Pv.length;yI++) {if (xI != yI && Pv[xI] == Pv[yI] && Pv[xI] != "") {var rgx = new RegExp("Pv\\["+yI+"\\]","g");Pv[xU] = Pv[xU].replace(rgx,"Pv["+xI+"]")}}} //consolidate identical pointers
	...
	function vAddS(xU,xL) { //parse cAdd into Sv
		xL += "";xU += "";
		if      (SvTest(xU) && !SvTest(xL)) {Sv[Sv.length-1] = Sv[Sv.length-1]+"_"+xL}
		else if (SvTest(xL) && !SvTest(xU)) {Sv[Sv.length-1] = Sv[Sv.length-1]+"_"+xU}
		else    {Sv.push(xU+"_"+xL)}
	...
	function vSubS(xU,xL) { //parse cSub into Sv
		...
		if (SvTest(xU) && !SvTest(xL)) {Sv[Sv.length-1] = Sv[Sv.length-1]+"_"+cNegS(xL)}
			...
			var tDif = eval(xL).split("_");
			Sv[Sv.length-1] = xU;
			for (var tD=0;tD<tDif.length;tD++) {Sv[Sv.length-1] = Sv[Sv.length-1]+"_"+cNegS(tDif[tD])}
		}
		else {Sv.push(xU+"_"+cNegS(xL))}
		return "Sv["+(Sv.length-1)+"]";
	}
	function vMulS(xU,xL) { //parse cMul into Pv
		xL += "";xU += "";
		if      (PvTest(xU) && !PvTest(xL)) {Pv[Pv.length-1] = Pv[Pv.length-1]+"_"+xL}
		else if (PvTest(xL) && !PvTest(xU)) {Pv[Pv.length-1] = Pv[Pv.length-1]+"_"+xU}
		else    {Pv.push(xU+"_"+xL)}
	...
	function pNomial(pN,pVar) { //parse polynomial to terms
		pN += "";
	...
	function pVariable(vP) { //ID dominant polynomial variable
		...
		var pTest = [];
		var vReturn = "";
		var sCount = (vP.split(/Cv\[\d+\]/).length-1);
	...
	function pDivide(xU,xL) { //polynomial division
		xU += "";xL += "";
		var fGcf = cGcf(aGcf(pCoeff(pNomial(xU))),aGcf(pCoeff(pNomial(xL))));
		if (fGcf != 0 && fGcf != 1) {xU = xReduce(cDivS(xU,fGcf)); xL = xReduce(cDivS(xL,fGcf))}
		var pVar = pVariable(xU);
		var polyU = pNomial(xU,pVar);
		var polyL = pNomial(xL,pVar);	
		if (!strTest(xL,pVar)) {pVar = pVariable(xL)}
		if (!strTest(xU,pVar)) {return "cDiv("+xU+","+xL+")"}	
		...
		for (var pT=0;pT<pDegree;pT++) {
			...
			xTractU = opExtract(uTemp);
			if (+uTemp == 0 || !strTest(uTemp,pVar) || strTest(uTemp,"cDiv(")) {break}
		}
		dReturn = xReduce(cAddS(dReturn,cDivS(uTemp,xL)));
		if (dReturn == "undefined") {return "cDiv("+xU+","+xL+")"}
	...
	function cPowS(xU,xL) {
		...
		var xTractU = opExtract(xU);
		var xTractL = opExtract(xL);
		if (xTractU.func == "mat" && +xL == int(+xL)) {
			if (+xL < -1)  {return "undefined"} //undefined
			if (+xL == -1) {return "undefined"} //inverse matrix
			if (+xL == 0)  {return matFunc(idn(matArray(xU)))} //identity matrix
			var mReturn = xU
		...
		if (+xU == 0) {return 0}
		if (+xL == 0) {return 1}
		if (+xL == .5 || (xTractL.func == "cDiv" && +xTractL.upper == 1 && +xTractL.lower == 2)) {return sqtS(xU)}
		...
		if (+xL == -.5 || (xTractL.func == "cDiv" && +xTractL.upper == -1 && +xTractL.lower == 2) || (xTractL.func == "cDiv" && +xTractL.upper == 1 && +xTractL.lower == -2)) {return cDivS(1,sqtS(xU))}
		if (!pxpFlag && +xL == 1.5 && nbrTest(xU)) {return cMulS(xU,sqtS(xU))}
		if (!pxpFlag && +xL*2 == Math.floor(+xL*2) && +xL != Math.floor(+xL)) {return cPowS(xU,cDivS(+xL*2,2))}
		if (xU == "Cv[46]" && +xL == Math.floor(+xL) && mgConfig.Domain == "Complex") {return (fmtResult(cPow(eval(xU),xL)))}
		if (xU == "Cv[8]" && cIterate(xL) == "cMul(Cv[46],Cv[29])") {return -1}
		if (nbrTest(xU) && nbrTest(xL) && cPow(xU,xL) == int(cPow(xU,xL))) {return (fmtResult(cPow(xU,xL)))}
	...
	function cMulS(xU,xL) {
		...
		var xTractU = opExtract(xU);
		var xTractL = opExtract(xL);
		if (xTractU.func == "mat" && xTractL.func != "mat") { //matrix scalar multiply
			xU = matArray(xU);
			var mReturn = xU;
			for (var iR=0;iR<xU.length;iR++) {
				for (var iC=0;iC<xU[iR].length;iC++) {mReturn[iR][iC] = cMulS(xU[iR][iC],xL)}
			}
			return matFunc(mReturn)
		}
		if (xTractL.func == "mat" && xTractU.func != "mat") { //matrix scalar multiply
			xL = matArray(xL);
			var mReturn = xL;
			for (var iR=0;iR<xL.length;iR++) {
				for (var iC=0;iC<xL[iR].length;iC++) {mReturn[iR][iC] = cMulS(xL[iR][iC],xU)}
			}
			return matFunc(mReturn)
		}
		if (xTractU.func == "mat" && xTractL.func == "mat") { //matrix dot product
			...
			for (var rU=0;rU<rowsU;rU++) {mReturn[rU] = new Array(colsL);mReturn[rU] = Array.apply(null, Array(colsL)).map(Number.prototype.valueOf,0)}
		...
		if (xU == "Cv[8734]" || xL == "Cv[8734]" ) {return "Cv[8734]"}
		if (nbrTest(xU) && nbrTest(xL)) {return (+xU)*(+xL)}
		...
		if (!factorFlag && xTractL.func == "cMul" && nbrTest(xU) && nbrTest(xTractL.upper)) {return cMulS(xU*xTractL.upper,xTractL.lower)}
		...
		if (xTractU.func == "cDiv" && xTractL.func != "cDiv") {return cDivS(cMulS(xL,xTractU.upper),xTractU.lower)}
		if (xTractL.func == "cDiv" && xTractU.func != "cDiv") {return cDivS(cMulS(xU,xTractL.upper),xTractL.lower)}
		if (xTractU.func == "cDiv" && xTractL.func == "cDiv") {return cDivS(cMulS(xTractU.upper,xTractL.upper),cMulS(xTractU.lower,xTractL.lower))}
	...
	function cDivS(xU,xL) {
		...
		var xTractU = opExtract(xU);
		var xTractL = opExtract(xL);
		var xTractD = opExtract(xTractL.lower)
		...
		if (nbrTest(xU) && nbrTest(xL) && (+xU)/(+xL) == Math.floor((+xU)/(+xL))) {return (+xU)/(+xL)}
		if (nbrTest(xU) && nbrTest(xL)) {var gTmp = cGcf(+xU,+xL);if (gTmp > 1) {return cDivS(+xU/gTmp,+xL/gTmp)}}		
		if (xTractU.func == "cMul" && nbrTest(xTractU.upper) && nbrTest(xL)) {
			var gTmp = cGcf(+xTractU.upper,+xL);
			if (gTmp > 1) {return cDivS(cMulS(+xTractU.upper/gTmp,xTractU.lower),+xL/gTmp)}
		}
		if (!factorFlag && pxpFlag && xTractL.func == "cMul") {
			if (pNomial(xTractL.upper).length < pNomial(xTractL.lower).length) {return "cDiv(cDiv("+xU+","+xTractL.upper+"),"+xTractL.lower+")"}
			return "cDiv(cDiv("+xU+","+xTractL.lower+"),"+xTractL.upper+")"
		}
		if (xTractU.func == "cAdd" || xTractU.func == "cSub") {xU = "("+xU+")"}
		if (xTractL.func == "cAdd" || xTractL.func == "cSub") {xL = "("+xL+")"}
		if (xTractU.func == "cAdd" && (pNomial(xL).length < 2 || factorFlag || limitFlag)) {return cAddS(cDivS(xTractU.upper,xL),cDivS(xTractU.lower,xL))}
		if (pxpFlag && xTractU.func == "cSub" && (pNomial(xL).length < 2 || factorFlag || limitFlag)) {return cSubS(cDivS(xTractU.upper,xL),cDivS(xTractU.lower,xL))}
		if (xU == xL) {return "1"}
		if (xTractL.func == "cPow" && xTractU.func == "cPow" && xTractL.upper == xTractU.upper) {return cPowS(xTractU.upper,cSubS(xTractU.lower,xTractL.lower))}
		if (xTractL.func == "cPow" && xTractD.func == "cNeg") {return cMulS(xU,cPowS(xTractL.upper,xTractD.upper))}
		...
		if (xTractU.func == "cDiv") {return cDivS(xTractU.upper,cMulS(xTractU.lower,xL))}
		if ((xTractL.func == "cAdd" || xTractL.func == "cSub") && !limitFlag && !pxpFlag) {return pDivide(xU,xL)}	
	...
	function cAddS(xU,xL) {
		...
		if (xTractU.func == "cNeg" && xL.search(/Cv\[111\d+\]/) == -1) {return cSubS(xL,xTractU.upper)}
	...
	function cNegS(xU) {
		...
		if (xTractU.func == "cDiv") {
			var xTractT = opExtract(xTractU.upper);
			if (xTractT.func == "cSub") {return cDivS(cNegS(cSubS(xTractT.upper,xTractT.lower)),xTractU.lower)}
		}
	...
	function sqtS(xU) {
		var xTractU = opExtract(xU);
		if (+xU < 0 && mgConfig.Domain == "Complex") { //calculate complex roots
			var sqTemp = (fmtResult(sqt(xU)));
			return cFunc(sqTemp)
		}
		if (+xU < 0 && mgConfig.Domain != "Complex") {return "undefined"}
		if ( (nbrTest(xU) && sqt(xU) == int(sqt(xU))) || (nbrTest(xU) && +xU != int(+xU)) ){return (fmtResult(sqt(xU)))} //calculate numerical roots
		if (xTractU.func == "cDiv" && (nbrTest(xTractU.upper) || nbrTest(xTractU.lower))) {return cDivS(sqtS(xTractU.upper),sqtS(xTractU.lower))}
	...
	function cbtS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU) && cbt(xU) == int(cbt(xU)) ) {return (fmtResult(cbt(xU)))} //calculate numerical roots
	...
	function nrtS(xU,xL)  {
		...
		if (nbrTest(xU) && nbrTest(xL) && nrt(xU,xL) == int(nrt(xU,xL))) {return (fmtResult(nrt(xU,xL)))}
	...
	function lneS(xU) {
		...
		if (nbrTest(xU) && lne(xU) == int(lne(xU)) ) {return (fmtResult(lne(xU)))}
	...
	function logS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU) && log(xU) == int(log(xU))) {return (fmtResult(log(xU)))}
	...
	function lgnS(xU,xL)  {
		...
		if (nbrTest(xU)  && nbrTest(xL) && lgn(xU,xL) == int(lgn(xU,xL))) {return (fmtResult(lgn(xU,xL)))}
	...
	function sinS(xU) {
		...
		if (+xU == 0) {return "0"}
		if (nbrTest(xU)) {return (fmtResult(sin(xU)))}
		return "sin("+xU+")"
	}
	var cosAngle = [
	...
	function cosS(xU) {
		...
		if (+xU == 0) {return "1"}
		if (nbrTest(xU)) {return (fmtResult(cos(xU)))}	
		return "cos("+xU+")"
	}
	var tanAngle = [
	...
	function tanS(xU) {
		...
		if (+xU == 0) {return "0"}
		if (nbrTest(xU)) {return (fmtResult(tan(xU)))}
		return "tan("+xU+")"
	}
	var secAngle = [
	...
	function secS(xU) {
		...
		if (+xU == 0) {return "1"}
		if (nbrTest(xU)) {return (fmtResult(sec(xU)))}
		return "sec("+xU+")"
	}
	var cscAngle = [
	...
	function cscS(xU) {
		...
		if (nbrTest(xU)) {return (fmtResult(csc(xU)))}
		return "csc("+xU+")"
	}
	var cotAngle = [
	...
	function cotS(xU) {
		...
		if (+xU == 0) {return "undefined"}
		if (nbrTest(xU)) {return (fmtResult(cot(xU)))}
		return "cot("+xU+")"
	}
	function asnS(xU) {
		var xTractU = opExtract(xU);
		if (strTest(sinAngle,xU)) {return cMulS(invMult,iAngle[sinAngle.indexOf(xU)])}
		if (nbrTest(xU)) {return (fmtResult(asn(xU)))}
	...
	function acsS(xU) {
		...
		if (nbrTest(xU)) {return (fmtResult(acs(xU)))}
	...
	function atnS(xU) {
		...
		if (nbrTest(xU)) {return (fmtResult(atn(xU)))}
	...
	function ascS(xU) {
		...
		if (nbrTest(xU)) {return (fmtResult(asc(xU)))}
	...
	function accS(xU) {
		...
		if (nbrTest(xU)) {return (fmtResult(acc(xU)))}
	...
	function actS(xU) {
		...
		if (nbrTest(xU)) {return (fmtResult(act(xU)))}
	...
	function snhS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU)) {return (fmtResult(snh(xU)))}
	...
	function cshS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU)) {return (fmtResult(csh(xU)))}
	...
	function tnhS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU)) {return (fmtResult(tnh(xU)))}
	...
	function schS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU)) {return (fmtResult(sch(xU)))}
	...
	function cchS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU)) {return (fmtResult(cch(xU)))}
	...
	function cthS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU)) {return (fmtResult(cth(xU)))}
	...
	function ashS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU)) {return (fmtResult(ash(xU)))}
		if (xTractU.func == "snh") {return xTractU.upper}
		return "ash("+xU+")"
	}
	function achS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU)) {return (fmtResult(ach(xU)))}
		if (xTractU.func == "csh") {return xTractU.upper}
		return "ach("+xU+")"
	}
	function athS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU)) {return (fmtResult(ath(xU)))}
	...
	function axhS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU)) {return (fmtResult(axh(xU)))}
		if (xTractU.func == "sch") {return xTractU.upper}
		return "axh("+xU+")"
	}
	function ayhS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU)) {return (fmtResult(ayh(xU)))}
		if (xTractU.func == "cch") {return xTractU.upper}
		return "ayh("+xU+")"
	}
	function azhS(xU) {
		var xTractU = opExtract(xU);
		if (nbrTest(xU)) {return (fmtResult(azh(xU)))}
	...
	function parseCalculus(dExp) { //parse calculus in MG format to FUNC: d/dx(x^2) = idr(x)x^2 -> drv(x^2,x)
		...
		if (strTest(dExp,"Cv[8748]")) {return cError("<span style='color:#880000'>Illegal differential</span>")}
		...
		for (var nC=0;nC<sCount;nC++) {
			if (!strTest(dExp,"dif(")) {
				...
				else {return cError("<span style='color:#880000'>Missing <i>dx</i></span>")}
			}
		...
		for (nC=0;nC<sCount;nC++) {
			...
			if (strTest(dExp,"(,") || strTest(dExp,",)")) {return cError("<span style='color:#880000'>Missing integral limit(s)</span>")}
		...
		for (var nC=0;nC<sCount;nC++) {
			...
			if (strg.inside.charAt(strg.inside.length-1) == "," || strg.inside == "") {return cError("<span style='color:#880000'>Missing operand(s)</span>")}
	...
	function drvS(dXpr,deeVar) { //partial derivative
		...
		dXpr += ""
		if (dXpr == "undefined") {return "undefined"}
	...
	function ntpS(nXpr,deeVar,iU,iL) { //wrapper for unsolved integral
		if (typeof iU != "undefined" && typeof iL != "undefined") {
			iU += "";iL += "";
	...
	function ntgS(nXpr,deeVar,iU,iL) {
		function ntgCheck(rCheck) {if (typeof rCheck == "undefined" || strTest(rCheck,"Cv[9998]") || strTest(rCheck,"undefined")) {return false}; return true} //check for ntg success
		...
		function cMulI(xU,xL) {
			...
			if (xTractL.func == "lne" && strTest(xU,deeVar) && strTest(xL,deeVar)) {
				var iTemp = cSubS(cDivS(cMulS(cPowS(xU,2),lneS(xU)),2),cDivS(cPowS(xU,2),4));
				...
				if (xReduce(cMulS(xU,xL)) == drvS(iTemp,deeVar)) {return iTemp}
				if (xTractU.func=="cPow" && xTractU.upper == deeVar && xTractL.upper == deeVar) {
					return cMulS(cPowS(xTractL.upper,cAddS(xTractU.lower,1)),cSubS(cDivS(lneS(xTractL.upper),cAddS(xTractU.lower,1)),cDivS(1,cPowS(cAddS(xTractU.lower,1),2))));
				}
			}
			if (xTractU.func == "lne" && xTractL.func != "lne" && strTest(xU,deeVar) && strTest(xL,deeVar)) {return ntgS(cPowS(xL,xU),deeVar)}
			if (xTractL.func == "cMul" && strTest(xU,deeVar) && strTest(xTractL.lower,deeVar) && !strTest(xTractL.upper,deeVar)) {return cMulS(xTractL.upper,ntgS(cMulS(xU,xTractL.lower),deeVar))}
			if (xTractL.func == "cMul" && strTest(xU,deeVar) && strTest(xTractL.upper,deeVar) && !strTest(xTractL.lower,deeVar)) {return cMulS(xTractL.lower,ntgS(cMulS(xU,xTractL.upper),deeVar))}
			...
			if (strTest(xL,deeVar) && strTest(xU,deeVar)) {
				if (xTractU.func == "cos" && xTractL.func == "sin" && xTractU.upper == deeVar && xTractL.upper == deeVar) {return cNegS(cDivS(cPowS(cosS(deeVar),2),2))}
				if (xTractU.func == "sec" && xTractL.func == "tan" && xTractU.upper == xTractL.upper) {return iSearch(cMulS(xU,xL),"sec")}
				if (xTractU.func == "cot" && xTractL.func == "csc" && xTractU.upper == xTractL.upper) {return iSearch(cMulS(xU,xL),"csc")}
				if (xTractU.func == "sch" && xTractL.func == "tnh" && xTractU.upper == xTractL.upper) {return iSearch(cMulS(xU,xL),"sch")}
				if (xTractU.func == "cch" && xTractL.func == "cth" && xTractU.upper == xTractL.upper) {return iSearch(cMulS(xU,xL),"cch")}
				if (xTractU.func == "cPow" && xTractL.func == "cos" && xTractU.upper == "Cv[8]" && xTractU.lower == deeVar && xTractL.upper == deeVar) {return cDivS(cMulS(cPowS("Cv[8]",deeVar),cAddS(sinS(deeVar),cosS(deeVar))),2)}
				if (xTractU.func == "cPow" && xTractL.func == "sin" && xTractU.upper == "Cv[8]" && xTractU.lower == deeVar && xTractL.upper == deeVar) {return cDivS(cMulS(cPowS("Cv[8]",deeVar),cSubS(sinS(deeVar),cosS(deeVar))),2)}	
				var sTemp = iSubst(xU,xL);
				if (ntgCheck(sTemp)) {return sTemp}
				sTemp = iParts(xU,xL);
				if (ntgCheck(sTemp)) {return sTemp}
				sTemp = iCheck(xU,xL);
				if (ntgCheck(sTemp)) {return sTemp}
		...
		function cDivI(xU,xL) {
			...
			if (deeVar == xL && !strTest(xU,deeVar)) {
				if (mgConfig.Domain == "Real") {return cMulS(xU,lneS(absS(cIterate(xL))))}
				return cMulS(xU,lneS(cIterate(xL)))
			}
			...
			if (strTest(xL,"sqt") && strTest(xL,deeVar) && strTest(xU,deeVar)) {
				var iTemp = xReduce(cDivS(cDivS(xU,xL),drvS(xL,deeVar)));
				if (drvS(cMulS(iTemp,xL),deeVar) == cDivS(xU,xL)) {return cMulS(iTemp,xL)}
			}
			if (strTest(xL,deeVar) && !strTest(xU,deeVar)) {
				if (xprMatch(xL,"cPow(Cv[8],cPow(Cv[9999],2))") != "" )	{return iSearch(cDivS(xU,xL),"erf")}
				...
				var iReturn = cMulS(xU,lneS(cIterate(xL)))
				var tTest = cDivS(drvS(iReturn,deeVar),cDivS(xU,cIterate(xL)));
				if (!strTest(tTest,deeVar) && ntgCheck(tTest)) {return cDivS(iReturn,tTest)}
				...
				if (xTractL.func == "cos") {return cMulS(secI(xTractL.upper),xU)}
				if (xTractL.func == "sin") {return cMulS(cscI(xTractL.upper),xU)}
				if (xTractL.func == "csh") {return cMulS(schI(xTractL.upper),xU)}
				if (xTractL.func == "snh") {return cMulS(cchI(xTractL.upper),xU)}		
				if (xprMatch(xL,"sqt(cSub(1,cPow(Cv[9999],2)))") != "")			{return iSearch(cDivS(xU,xL),"asn")}
				if (xprMatch(xL,"cNeg(sqt(cSub(1,cPow(Cv[9999],2))))") != "")	{return iSearch(cDivS(xU,xL),"acs")}
				if (xprMatch(xL,"sqt(cAdd(cPow(Cv[9999],2),1))") != "") 		{return iSearch(cDivS(xU,xL),"ash")}
				if (xprMatch(xL,"sqt(cSub(cPow(Cv[9999],2),1))") != "") 		{return iSearch(cDivS(xU,xL),"ach")}
				if (xprMatch(xL,"cAdd(cPow(Cv[9999],2),1)") != "") 				{return iSearch(cDivS(xU,xL),"atn")}
				if (xprMatch(xL,"cSub(cPow(Cv[9999],2),1)") != "") 				{return iSearch(cDivS(xU,xL),"ath")}
				if (xTractL.func == "sqt") 	{var iTemp = cMulS(xU,cPowI(xTractL.upper,"cDiv(-1,2)")); if (ntgCheck(iTemp)) {return iTemp}}	
				if (strTest(xL,"sqt")) {
			...
			if (strTest(xL,deeVar) && pNomial(xL).length > 2 && iIterations < 2) { //integration by partial fractions
				...
				if (fTemp != "cDiv("+xU+","+xL+")") {
					...
					if (ntgCheck(pTemp)) {return pTemp}
				}
			}
			if (strTest(xL,deeVar) && strTest(xU,deeVar)) {
				var pTemp = iSubst(xU,"cPow("+xL+",-1)");
				if (ntgCheck(pTemp)) {return pTemp}
				pTemp = iParts(xU,"cDiv(1,"+xL+")");
				if (ntgCheck(pTemp)) {return pTemp}
			}
			return "undefined"
		}
		function lneI(xU) {
			...
			var lTemp = cAddS(cMulS(xTractU.lower,lneS(xU)),xTractU.lower); //division deeVar quotient
			if (lneS(xU) == drvS(lTemp,deeVar)){return lTemp}
		...
		function sqtI(xU)    {
			...
			var uTemp = ntgS(cPowS(xU,"cDiv(1,2)"),deeVar);
			if (ntgCheck(uTemp)) {return uTemp}
		...
		function cosI(xU) {
			...
			return iTest(cosS(xU))
		}
		function tanI(xU) {
			if (deeVar == xU) {
				if (mgConfig.Domain == "Real") {cNegS(lneS(absS(cosS(xU))))}
				return cNegS(lneS(cosS(xU)))
			}
			return iTest(tanS(xU))
		}
		function cscI(xU) {
			if (deeVar == xU) {
				if (mgConfig.Domain == "Real") {return cSubS(lneS(absS(sinS(cDivS(xU,2)))),lneS(absS(cosS(cDivS(xU,2)))))}
				return cSubS(lneS(sinS(cDivS(xU,2))),lneS(cosS(cDivS(xU,2))))
			}
			return iTest(cscS(xU))
		}
		function secI(xU) {
			if (deeVar == xU) {
				if (mgConfig.Domain == "Real") {return lneS(absS(cAddS(secS(xU),tanS(xU))))}
				return lneS(cAddS(secS(xU),tanS(xU)))
			}
			return iTest(secS(xU))
		}
		function cotI(xU) {
			if (deeVar == xU) {
				if (mgConfig.Domain == "Real") {return lneS(absS(sinS(xU)))}
				return lneS(sinS(xU))
			}
			return iTest(cotS(xU))
		}
		function snhI(xU) {
			var xTractU = opExtract(xU);
		...
		function cshI(xU) {
			...
			return iTest(cshS(xU))
		}
		function tnhI(xU) {
			var xTractU = opExtract(xU);
			if (deeVar == xU) {return lneS(cshS(xU))}
			if (xTractU.func == "lne" && xTractU.upper == deeVar) {return cSubS(deeVar,cMulS(2,atnS(deeVar)))}
			return iTest(tnhS(xU))
		}
		function schI(xU) {if (deeVar == xU) {return cMulS(2,atnS(snhS(cDivS(xU,2))))};return iTest(schS(xU))}
		function cchI(xU) {
			if (deeVar == xU) {
				if (mgConfig.Domain == "Real") {return lneS(absS(tnhS(cDivS(xU,2))))}
				return lneS(tnhS(cDivS(xU,2)))
			}
			return iTest(cchS(xU))
		}
		function cthI(xU) {
			if (deeVar == xU) {
				if (mgConfig.Domain == "Real") {return lneS(absS(snhS(xU)))}
				return lneS(snhS(xU))
			}
			return iTest(cthS(xU))
		}
		function asnI(xU) {if (deeVar == xU) {return cAddS(cMulS(xU,asnS(xU)),sqtS(cSubS(1,cPowS(xU,2))))};return iTest(asnS(xU))}
		function acsI(xU) {if (deeVar == xU) {return cSubS(cMulS(xU,acsS(xU)),sqtS(cSubS(1,cPowS(xU,2))))};return iTest(acsS(xU))}
		function atnI(xU) {if (deeVar == xU) {return cSubS(cMulS(xU,atnS(xU)),cDivS(lneS(cAddS(1,cPowS(xU,2))),2))};return iTest(atnS(xU))}
		function actI(xU) {if (deeVar == xU) {return cAddS(cMulS(xU,atnS(xU)),cDivS(lneS(cAddS(1,cPowS(xU,2))),2))};return iTest(actS(xU))}
		function ascI(xU) {if (deeVar == xU) {return cSubS(cMulS(xU,ascS(xU)),atnS(sqtS(cSubS(1,cDivS(1,cPowS(xU,2))))))};return iTest(ascS(xU))}
		function accI(xU) {if (deeVar == xU) {return cSubS(cMulS(xU,acsS(xU)),sqtS(cSubS(1,cPowS(xU,2))))};return iTest(accS(xU))}
		function ashI(xU) {if (deeVar == xU) {return cSubS(cMulS(xU,ashS(xU)),sqtS(cAddS(1,cPowS(xU,2))))};return iTest(ashS(xU))}
		function achI(xU) {if (deeVar == xU) {return cSubS(cMulS(xU,achS(xU)),sqtS(cSubS(cPowS(xU,2),1)))};return iTest(achS(xU))}
		function athI(xU) {if (deeVar == xU) {return cAddS(cMulS(xU,athS(xU)),cDivS(lneS(cSubS(cPowS(xU,2),1)),2))};return iTest(athS(xU))}
		function axhI(xU) {return "undefined"}
		function ayhI(xU) {return "undefined"}
		function azhI(xU) {if (deeVar == xU) {return cAddS(cDivS(lneS(cSubS(1,cPowS(xU,2)))),cMulS(xU,azhS(xU)))};return iTest(azhS(xU))}
		function expI(xU) {if (deeVar == xU) {return expS(xU)};return iTest(expS(xU))}
		function absI(xU) {if (deeVar == xU) {return "cDiv(cMul("+xU+",abs("+xU+")),2)"};return iTest(absS(xU))}
		function erfI(xU) {if (deeVar == xU) {return "cAdd(cMul("+xU+",erf("+xU+")),cDiv(1,cMul(cPow(Cv[8],cPow("+xU+",2)),sqt(Cv[29]))))"};return iTest(erfS(xU))}
		function efcI(xU) {return "undefined"}
		function lgnI(xU,xL) {if (deeVar == xL) {return cSubS(cMulS(xL,lgnS(xU,xL)),cDivS(xL,lneS(xU)))};return "undefined"}
		...
		function iParts(xU,xL) { //integration by parts
			...
			var drvTmpL = xL;
			var iReturn = "0";
			for (var dT=0;dT<10;dT++) {drvTmpU = drvS(drvTmpU,deeVar);if (drvTmpU.length > xU.length*3 || +drvTmpU == 0){break}}
			for (var dT=0;dT<10;dT++) {drvTmpL = drvS(drvTmpL,deeVar);if (drvTmpL.length > xL.length*3 || +drvTmpL == 0){break}}
			if      (+drvTmpU == 0 && ntgCheck(ntgS(xL,deeVar))) {dTable[0] = xU;iTable[0] = xL}
			else if (+drvTmpL == 0 && ntgCheck(ntgS(xU,deeVar))) {dTable[0] = xL;iTable[0] = xU}
			else    {return "undefined"}
			if (drvTmpU != "0" && drvTmpL != "0") {return "undefined"}
			for (var dT=1;dT<11;dT++) {
				dTable[dT] = drvS(dTable[dT-1],deeVar);
				iTable[dT] = ntgS(iTable[dT-1],deeVar);
				if (+dTable[dT] == 0) {break}
			}
			for (var dT=0;dT<iTable.length-1;dT++) {
				if (dT/2 == Math.floor(dT/2)) {iReturn = cAddS(iReturn,cMulS(dTable[dT],iTable[dT+1]))}
				else {iReturn = cSubS(iReturn,cMulS(dTable[dT],iTable[dT+1]))}
			}
			if (ntgCheck(iReturn) && !strTest(iReturn,"ntp")) {return iReturn}
			return "undefined"
		}
		function iSubst(xU,xL) { //integration by cMul substitution 
			var xTractU = opExtract(xU);
			if (xTractU.func == ""){xTractU.upper = xU}
			var xTractL = opExtract(xL);
			if (xTractL.func == ""){xTractL.upper = xL}
			var factorTmpU1 = xReduce(cDivS(xL,drvS(xTractU.upper,deeVar)));
			if (ntgCheck(factorTmpU1) && !strTest(factorTmpU1,deeVar)) {return cMulS(factorTmpU1,ntgS(xU,xTractU.upper))}
			var factorTmpL1 = xReduce(cDivS(xU,drvS(xTractL.upper,deeVar)));
			if (ntgCheck(factorTmpL1) && !strTest(factorTmpL1,deeVar)) {return cMulS(factorTmpL1,ntgS(xL,xTractL.upper))}
			var factorTmpU2 = xReduce(cDivS(xL,drvS(xTractU.lower,deeVar)));
			if (ntgCheck(factorTmpU2) && !strTest(factorTmpU2,deeVar)) {return cMulS(factorTmpU2,ntgS(xU,xTractU.lower))}
			var factorTmpL2 = xReduce(cDivS(xU,drvS(xTractL.lower,deeVar)));
			if (ntgCheck(factorTmpL2) && !strTest(factorTmpL2,deeVar)) {return cMulS(factorTmpL2,ntgS(xL,xTractL.lower))}
			return "undefined"
		}
		function uSubst(xU) { //integration by u substitution
			var xTractU = opExtract(xU);
			if (xTractU.func == "") {return "undefined"}
			if (strTest(xTractU.upper,deeVar) && strTest(xTractU.lower,deeVar)) {return "undefined"}	
			var xVar = cFunc(xprSolve(mgExport(xU)+"="+"Cv[9999]",deeVar).split("=")[1]);
			var uReturn = cSubst(ntgS(cMulS("Cv[9999]",drvS(xVar,"Cv[9999]")),"Cv[9999]"),"Cv[9999]",xU);
			if (xU == drvS(uReturn,deeVar)) {return uReturn}
			return "undefined"
		}
		function iCheck(xU,xL) { //integration by operand check
			var xTractU = opExtract(xU);
			var xTractL = opExtract(xL);
			var iReturn = 1;
			if (strTest(xTractU.lower,deeVar)) {
				iReturn = ntgS("cMul("+xU+","+xL+")",xTractU.lower);
				var xTemp = xReduce(cDivS("cMul("+xU+","+xL+")",drvS(iReturn,deeVar)))
				if (ntgCheck(xTemp) && !strTest(xTemp,deeVar)) {return cMulS(xTemp,iReturn)}			
			}
			if (strTest(xTractU.upper,deeVar)) {
				iReturn = ntgS("cMul("+xU+","+xL+")",xTractU.upper); 
				var xTemp = xReduce(cDivS("cMul("+xU+","+xL+")",drvS(iReturn,deeVar)))
				if (ntgCheck(xTemp) && !strTest(xTemp,deeVar)) {return cMulS(xTemp,iReturn)}	
			}
			if (strTest(xTractL.lower,deeVar)) {
				iReturn = ntgS("cMul("+xU+","+xL+")",xTractU.lower); 
				var xTemp = xReduce(cDivS("cMul("+xU+","+xL+")",drvS(iReturn,deeVar)))
				if (ntgCheck(xTemp) && !strTest(xTemp,deeVar)) {return cMulS(xTemp,iReturn)}
			}
			if (strTest(xTractL.upper,deeVar)) {
				iReturn = ntgS("cMul("+xU+","+xL+")",xTractU.upper); 
				var xTemp = xReduce(cDivS("cMul("+xU+","+xL+")",drvS(iReturn,deeVar)))
				if (ntgCheck(xTemp) && !strTest(xTemp,deeVar)) {return cMulS(xTemp,iReturn)}
			}
		...
		function iTest(xU,xD) { //fn integration by test
			var xTractU = opExtract(xU);
			if (typeof xD == "undefined") {xD = xTractU.upper}
			var iReturn = ntgS(xU,xD);
			var tTest = xReduce(cDivS(xU,drvS(iReturn,deeVar)));
			if (ntgCheck(tTest) && !strTest(tTest,deeVar)) {return cMulS(iReturn,tTest)}
			return "undefined"
		}
		function iSearch(xU,xF) { //integration by derivative search
			var tElements = cDissect(xU);
			var tElementsLen = tElements.length;
			...
			if (tElementsLen <= 10) {
				for (var tL=0;tL<tElementsLen;tL++) {
					if (tElements[tL] != deeVar && !nbrTest(tElements[tL]) && tElements[tL] != "") {
						...
						if (!strTest(tSearch,deeVar) && ntgCheck(tSearch)) {return cMulS(xF+"("+cSubS(deeVar,tElements[tL])+")",tSearch)}
						tSearch = xReduce(cDivS(xU,drvQ(xF+"("+cSubS(tElements[tL],deeVar)+")",deeVar)));
						if (!strTest(tSearch,deeVar) && ntgCheck(tSearch)) {return cMulS(xF+"("+cSubS(tElements[tL],deeVar)+")",tSearch)}
						tSearch = xReduce(cDivS(xU,drvQ(xF+"("+cMulS(deeVar,tElements[tL])+")",deeVar)));
						if (!strTest(tSearch,deeVar) && ntgCheck(tSearch)) {return cMulS(xF+"("+cMulS(deeVar,tElements[tL])+")",tSearch)}
						tSearch = xReduce(cDivS(xU,drvQ(xF+"("+cDivS(deeVar,tElements[tL])+")",deeVar)));
						if (!strTest(tSearch,deeVar) && ntgCheck(tSearch)) {return cMulS(xF+"("+cDivS(deeVar,tElements[tL])+")",tSearch)}
						tSearch = xReduce(cDivS(xU,drvQ(xF+"("+cDivS(tElements[tL],deeVar)+")",deeVar)));
						if (!strTest(tSearch,deeVar) && ntgCheck(tSearch)) {return cMulS(xF+"("+cDivS(tElements[tL],deeVar)+")",tSearch)}
					}
				}
			}
			return "undefined"
		...
		//
		if (iIterations > 30) {return "undefined"} //break infinite loop
		iIterations++;
		nXpr += "";
		if (strTest(nXpr,"ntp")) {return nXpr}
		if (typeof iU != "undefined" && typeof iL != "undefined") { //definite integral
			iU += "";iL += "";
			var iTmp = ntgS(xReduce(nXpr),deeVar);
			if (ntgCheck(iTmp) && !strTest(iTmp,"ntp(")) {
			...
			var dXpr = eval(xReduce(nXpr).replace(/([a-z])\(/,"$1I(").replace(/([a-z])\(/g,"$1S(").replace(/(Cv\[\d+\])/g,"'$1'").replace(/(Pv\[\d+\])/g,"'$1'").replace(/(Sv\[\d+\])/g,"'$1'")+"");
			if (ntgCheck(dXpr)) {return cReduce(dXpr)}
			dXpr = eval(cExpand(nXpr).replace(/([a-z])\(/,"$1I(").replace(/([a-z])\(/g,"$1S(").replace(/(Cv\[\d+\])/g,"'$1'").replace(/(Pv\[\d+\])/g,"'$1'").replace(/(Sv\[\d+\])/g,"'$1'")+"");
			if (ntgCheck(dXpr)) {return cReduce(dXpr)}
			return "ntp("+nXpr+","+deeVar+")"
		}
	}
	...
	function lmtS(lXpr,lVar,xLim) {
		...
		function cDivL(xU,xL) {
			...
			if (polyU.length >= 1 && polyL.length >= 1 && ""+pVariable(xU) == ""+pVariable(xL) && strTest(xLim,"Cv[8734]")) {
		...
		lXpr = eval(lXpr.replace(/([a-z])\(/g,"$1S(").replace(/(Cv\[\d+\])/g,"'$1'").replace(/cDivS/g,"cDivL").replace(/(Pv\[\d+\])/g,"'$1'").replace(/(Sv\[\d+\])/g,"'$1'"))+"";
		lXpr = eval(lXpr.replace(/([a-z])\(/g,"$1L(").replace(/(Cv\[\d+\])/g,"'$1'").replace(/(Pv\[\d+\])/g,"'$1'").replace(/(Sv\[\d+\])/g,"'$1'"))+"";
		limitFlag = false;
	...
	function cFactor(cFac) {
		...
		function pFactor(xFac) { //factor polynomials
			...
			tReturn = xReduce(cMulS(fGcf,"cMul((cAdd(cMul("+sqrtA+","+cPowS(pVar,(polyU.length-1)/2)+"),"+sqrtB+")),(cSub(cMul("+sqrtA+","+cPowS(pVar,(polyU.length-1)/2)+"),"+sqrtB+")))"));
			if (cReduce(cExpand(tReturn)) == cReduce(xFac)) {return tReturn}
			...
			fCoeff = pCoeff(polyU);
			var pRoot = cPowS(pVar,(polyU.length-1)/2);
			var aPb = fAddMul(fCoeff[0],fCoeff[(polyU.length-1)/2],fCoeff[(polyU.length-1)]);
			var gcfA = cGcf(fCoeff[(polyU.length-1)],aPb);
			var gcfB = cGcf(fCoeff[(polyU.length-1)/2]-aPb,fCoeff[0]);	
			...
			if (cExpand(tReturn) == cReduce(xFac) && fGcf != 1) {return tReturn}
			return xFac
		}
		function dtFactor(pfFac) { //factor div and trm
			pfFac += "";
			...
			if (xTract.func == "cDiv" && pNomial(xTract.lower).length > pNomial(xTract.upper).length) {//proper partial fractions
				...
				if (termsL.length == 2 && (pVariable(xTract.upper) == "" || pVariable(xTract.upper) == pVariable(xTract.lower)) ) {
					...
					var Z1 = cFunc(xprSolve("0="+mgExport(termsL[0]),fVar).split("=")[1]);
					var Z2 = cFunc(xprSolve("0="+mgExport(termsL[1]),fVar).split("=")[1]);
	...
	function xprRange(xR)  {
		function nEqual(nZ,nC) {
			...
			for (var iZ=0;iZ<zVars.length;iZ++) {zArray[iZ] = xprSolve(nZ+"="+nC,"Cv["+zVars[iZ]+"]")}
			for (var iZ=0;iZ<zVars.length;iZ++) {
				if (!strTest(zArray[iZ],"Cv[8734]")) {
					zString = zString+zArray[iZ].split("=")[0]+"Cv[8800]"+zArray[iZ].split("=")[1];
		...
		function sdrR(xU) {return "sdr("+xU+")"}
		function psdR(xU) {return "psd("+xU+")"}
		function ddrR(xU) {return "ddr("+xU+")"}
		function pddR(xU) {return "pdd("+xU+")"}
		function difR(xU) {return "dif("+xU+")"}
		function sumR(xU) {return "sum("+xU+")"}
		function prdR(xU) {return "prd("+xU+")"}
		...
		for (var xC=0;xC<dArray.length;xC++) {if (!strTest(xArray,dArray[xC]) && dArray[xC] != "" && !strTest(dArray[xC],"undefined")) {xArray.push(dArray[xC])}}
		var rString = "";
	...
	function smmS(sXpr,sUpper,dV,sLower) {
		...
		function cAddM(xU,xL) {
			xU += "";xL += "";
			return xReduce(cAddS(smmS(xU,sUpper,dV,sLower),smmS(xL,sUpper,dV,sLower)))
		}
		function cSubM(xU,xL) {
			xU += "";xL += "";
			return xReduce(cSubS(smmS(xU,sUpper,dV,sLower),smmS(xL,sUpper,dV,sLower)))
		} 
		...
		function sdrM(xU) {return "undefined"}
		function psdM(xU) {return "undefined"}
		function ddrM(xU) {return "undefined"}
		function pddM(xU) {return "undefined"}
		function difM(xU) {return "undefined"}
		//
		sXpr += "";sUpper += "";dV += "";sLower += "";
	...
	function pmmS(pXpr,pUpper,dV,pLower) {
		...
		function sdrP(xU) {return "undefined"}
		function psdP(xU) {return "undefined"}
		function ddrP(xU) {return "undefined"}
		function pddP(xU) {return "undefined"}
		function difP(xU) {return "undefined"}
		//
		pXpr += "";pUpper += "";dV += "";pLower += "";
	...
	function fmtResult(xIn) { //format numerical output
		if (getType(xIn) == "real" || getType(xIn) == "complex") {
			...
			if (mgConfig.cplxFmt == "Polar") {
				...
				if (cIn.r+"" == "Infinity" || cIn.i+"" == "Infinity") {return "Infinity"}
				if (cIn.r+"" == "-Infinity" || cIn.i+"" == "-Infinity") {return "-Infinity"}
	...
	function roundDecTo(xD,xT) { //round decimal to xT digits
		...
		if (eval(xD) < 1 || xD >= 1e12) {xD = expNotation(xD)}
		xD += "";
		var yD = xD.replace(/.*\./,"");
		...
		else {return eval(xD.replace(/\.\d+/,uD))}
	}
	...
	function cMul(xU,xL) { //multiply by term
		...
		if (getType(xU) == "matrix" && getType(xL) != "matrix") { //matrix scalar multiply
			var mReturn = xU;
			for (var iR=0;iR<xU.length;iR++) {
				for (var iC=0;iC<xU[iR].length;iC++) {mReturn[iR][iC] = cMul(xU[iR][iC],xL)}
			}
			return mReturn
		}
		if (getType(xL) == "matrix" && getType(xU) != "matrix") { //matrix scalar multiply
			var mReturn = xL;
			for (var iR=0;iR<xL.length;iR++) {
				for (var iC=0;iC<xL[iR].length;iC++) {mReturn[iR][iC] = cMul(xL[iR][iC],xU)}
			}
			return mReturn
		}
	...
	function cPow(xU,xL) { //powers xU^xL
		...
		if (getType(xU) == "matrix" && getType(xL) == "real" && xL == int(xL)) {
			if (+xL < -1)  {return "undefined"} //undefined
			if (+xL == -1) {return "undefined"} //inverse matrix
			if (+xL == 0)  {return idn(xU)} //identity matrix
			var mReturn = xU
	...
	function idn(xU) { //identity matrix
		if (getType(xU) == "matrix") {
			var rowsU = xU.length;
			if (rowsU != xU[0].length) {return "undefined"}
			var mReturn = new Array(rowsU);
			for (var rU=0;rU<rowsU;rU++) {mReturn[rU] = new Array(rowsU);mReturn[rU] = Array.apply(null, Array(rowsU)).map(Number.prototype.valueOf,0)}
			for (var rU=0;rU<rowsU;rU++) {mReturn[rU][rU] = 1}
			return mReturn
		}
		return "undefined"
	}
	function trc(xU) { //trace matrix
		if (getType(xU) == "matrix") {
			var rowsU = xU.length;
			if (rowsU != xU[0].length) {return "undefined"}
			var mReturn = 0;
			for (var rU=0;rU<rowsU;rU++) {mReturn = cAdd(mReturn,xU[rU][rU])}
			return mReturn
		}
		return "undefined"		
	}
	...
	function sqt(xU) {//square root
		if (getType(xU) == "complex" && +xU.i == 0) {xU = +xU.r}
		if (Math.sqrt(xU) == Math.sqrt(xU)*1) {return Math.sqrt(xU)}
	...
	function cos(xU) { //cosine
		...
		if (getType(xU) == "real") {return Math.floor(Math.cos(toReal(xU)*mgConfig.trigBase)*dRound)/dRound}
		return "undefined"
	}
	function sin(xU) { //sine
		if (getType(xU) == "complex" && xU.i != 0) {return cMul(cMul(-1,Cv[46]),snh(cMul(Cv[46],cMul(xU,mgConfig.trigBase))))}
		if (getType(xU) == "real") {return Math.floor(Math.sin(toReal(xU)*mgConfig.trigBase)*dRound)/dRound}
		return "undefined"
	}
	...
	var Solve      = function(xprA,xprB) {return xprSolve(xprA,xprB)			};
