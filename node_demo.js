...
console.log(translate.mgTranslate("\\phi_n(\\kappa) =\\frac{1}{4\\pi^2\\kappa^2} \\int_0^\\infty\\frac{\\sin(\\kappa R)}{\\kappa R}\\frac{\\partial}{\\partial R}\\left[R^2\\frac{\\partial D_n(R)}{\\partial R}\\right]\\,dR").html)

console.log("\n-- Simplify '\\frac{y \\cos {x y} \\sin {x y}}{y \\cos^{2} {x y}}':\n")
console.log(calculate.mgSimplify("\\frac{y \\cos {x y} \\sin {x y}}{y \\cos^{2} {x y}}").latex)

console.log("\n-- Evaluate Integral '\\int  \\sqrt{5+ \\sqrt{x}} d x':\n")
console.log(calculate.mgSimplify("\\int  \\sqrt{5+ \\sqrt{x}} d x").latex)

console.log("\n-- Evaluate Summation '\\sum_{x = 4}^{ \\infty }  \\frac{n^{x}}{x !}'\n")
console.log(calculate.mgSimplify("\\sum_{x = 4}^{ \\infty }  \\frac{n^{x}}{x !}").latex)

console.log("\n-- Solve equation '\\frac{1}{ \\sigma  \\sqrt{2 \\pi }} e^{ \\frac{- \\left(x-m \\right)^{2}}{2 \\sigma^{2}}}=p' for m:\n")
console.log(calculate.mgSolve("\\frac{1}{ \\sigma  \\sqrt{2 \\pi }} e^{ \\frac{- \\left(x-m \\right)^{2}}{2 \\sigma^{2}}}=p","m").latex)

console.log("\n-- Factor '9 x^{2}+24 x y+16 y^{2}':\n")
console.log(calculate.mgFactor("9 x^2+24 x y+16 y^2").latex)

console.log("\n-- Calculate complex numerical '\\frac{8-8 \\imath }{2+ \\imath }':\n")
console.log(calculate.mgNumeric("\\frac{8-8 \\imath }{2+ \\imath }").latex)

console.log("\n-- Calculate matrix dot product '\\begin{bmatrix}3&4&2 \\end{bmatrix} \\times \\begin{bmatrix}13&9&7&15 \\\\8&7&4&6 \\\\6&4&0&3 \\end{bmatrix}':\n")
console.log(calculate.mgNumeric("\\begin{bmatrix}3&4&2 \\end{bmatrix} \\times \\begin{bmatrix}13&9&7&15 \\\\8&7&4&6 \\\\6&4&0&3 \\end{bmatrix}").latex)
